"""
Options chain management: fetch, filter, and cache 0DTE BTC options.
"""
...
@dataclass
class OptionInfo:
    symbol: str           # e.g. BTC-18MAR26-85000-P
    strike: float
    option_type: str      # C or P
    expiry_str: str       # e.g. 18MAR26
    bid: float = 0.0
    ask: float = 0.0
    mid: float = 0.0
    mark: float = 0.0
...
class OptionChain:
    """Manages the 0DTE options chain for BTC."""

    def __init__(self, exchange: BybitExchange) -> None:
        self._exchange = exchange
        self._puts: dict[float, OptionInfo] = {}   # strike -> OptionInfo
    ...
    async def refresh(self) -> int:
        """
        Fetch today's 0DTE chain and cache it.
        Returns the number of puts found.
        """
        ...
        for t in tickers:
            ...
            parts = symbol.split("-")
            # USDT-settled: BTC-18MAR26-85000-P-USDT (5 parts)
            # Coin-settled: BTC-18MAR26-85000-P      (4 parts)
            if len(parts) < 4:
                continue
        ...
        log.info(
            "chain_refreshed",
            expiry=self._exp_date_str,
            puts=len(self._puts),
            calls=len(self._calls),
        )
        return len(self._puts)

    def get_itm_puts(self, spot: float) -> list[OptionInfo]:
        """
        Return all ITM puts sorted by strike ascending.
        A put is ITM when strike > spot.
        """
        return sorted(
    ...
    def get_nearest_itm_put(self, spot: float) -> Optional[OptionInfo]:
        """
        Closest ITM put: the lowest strike that is still above spot.
        Per spec: if spot=71640 and strikes are 71500, 72000 → pick 72000.
        """
        itm = self.get_itm_puts(spot)
        if not itm:
            return None
        return itm[0]
    ...
    def get_call(self, strike: float) -> Optional[OptionInfo]:
    ...
    @property
    def all_calls(self) -> list[OptionInfo]:
        return sorted(self._calls.values(), key=lambda c: c.strike)

    @property
    def available_strikes(self) -> list[float]:
        return sorted(set(list(self._puts.keys()) + list(self._calls.keys())))
